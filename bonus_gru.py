...
def train(gru,oh_tweets, seq_length, chars_with_indices, alph_size, hidden_size,n_epochs,lr):
    ...
    e=np.random.randint(0,len(oh_tweets[0].T)-seq_length)
        ...
        for tweet in oh_tweets:
            ...
            while e< tweet_length:
                ...
                if iterations%10000==0:
                    my_string = generate_chars(chars_with_indices, alph_size,hidden_size, char_from_index(chars_with_indices,np.argwhere(X[:,-1]==1)[0][0]), gru, 140)
                    print(my_string)
                e = e + seq_length
                iterations += 1
                if end:
    ...
    my_string = generate_chars(chars_with_indices, alph_size,hidden_size, 'R', gru, 140)
    print(my_string)
    plt.show()
...
X = oh_tweets[0][:, 0: seq_length]
Y = oh_tweets[0][:,  1:seq_length+1]



np.random.seed(100)
gru=GRU(m,K)
sentence=generate_chars(chars_with_indices,K,m,'a',gru,10)
h_0=np.zeros((m,1))
...
'''
train(gru,oh_tweets,seq_length,chars_with_indices,K,m,20,lr)
